...
bool Plane::verify_loiter_time()
{
    ...
    if (loiter.start_time_ms == 0 || (quadplane.guided_mode_enabled() && !auto_state.vtol_loiter)) {
...
bool Plane::verify_landing_vtol_approach(const AP_Mission::Mission_Command &cmd)
{
    ...
    if (vtol_approach_s.approach_stage < APPROACH_LINE && loiter.start_time_ms == 0 && (reached_loiter_target() || (!auto_state.vtol_loiter && auto_state.wp_proportion > 1)))
...
bool Plane::verify_loiter_heading(bool init)
{
    ...
    if (! mission.get_next_nav_cmd(mission.get_current_nav_index() + 1,
                                   next_nav_cmd)) {
        next_nav_cmd.content.location = rally.calc_best_rally_or_home_location(current_loc, get_RTL_altitude_cm());
    }
