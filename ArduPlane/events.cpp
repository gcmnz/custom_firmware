...
void Plane::handle_battery_failsafe(const char *type_str, const int8_t action)
{
    switch ((Failsafe_Action)action) {
        ...
        case Failsafe_Action_Loiter_alt_QLand:
            if (quadplane.available() && !failsafe_in_landing_sequence()) {
        ...
        case Failsafe_Action_QLand:
            if (quadplane.available() && !failsafe_in_landing_sequence()) {
                if (is_positive(quadplane.batt_fs_rtl_limit_km) &&
                    current_loc.get_distance(rally.calc_best_rally_or_home_location(current_loc, get_RTL_altitude_cm())) < quadplane.batt_fs_rtl_limit_km * 1000.0f) {
