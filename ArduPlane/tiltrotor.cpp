...
void Tiltrotor::update_psc()
{
    if (max_angle_deg <= 0 ||
        !quadplane.in_vtol_mode() ||
        !motors->armed() ||
        quadplane.vel_forward.gain <= 0 ||
        plane.control_mode == &plane.mode_qstabilize ||
        plane.control_mode == &plane.mode_qhover ||
    ...
    int32_t tilt_max_cd = constrain_float(quadplane.vel_forward.gain, 0.0f, 1.0f) * max_angle_deg * 100;
    int32_t tilt_cd = constrain_int32(-plane.nav_pitch_cd, 0, tilt_max_cd);

    plane.nav_pitch_cd += tilt_cd;
    psc_tilt = tilt_cd / 9000.0f;
}
