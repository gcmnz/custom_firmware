...
bool Plane::set_mode(Mode &new_mode, const ModeReason reason)
{
    ...
    if (quadplane.available() && &new_mode == &mode_qland) {
        float height_above_ground_m = relative_ground_altitude(g.rangefinder_landing);
        if (quadplane.land_max_alt_m > 0 && height_above_ground_m > quadplane.land_max_alt_m + 15) {
            gcs().send_text(MAV_SEVERITY_ALERT, "Spiral QLAND at %dm", (int)height_above_ground_m);
            rally.set_override_loc(current_loc);
            bool rtl_ok = set_mode(mode_rtl, ModeReason::FENCE_BREACHED);
            rally.clear_override_loc();
            rtl.target_override = true;
            return rtl_ok;
        }
    }
...
bool Plane::ekf_has_absolute_position() const
{
    ...
    nav_filter_status filt_status = quadplane.inertial_nav.get_filter_status();
...
bool Plane::ekf_has_relative_position() const
{
