...
const AP_Param::GroupInfo QuadPlane::var_info_extra[] = {
    AP_GROUPINFO("BFS_RTL_LIM_KM", 5, QuadPlane, batt_fs_rtl_limit_km, 0),
    AP_GROUPEND
};
    ...
    ahrs(_ahrs)
{
    ...
    AP_Param::setup_object_defaults(this, var_info_extra);
...
bool QuadPlane::setup(void)
{
    ...
    atc_var_info = AC_AttitudeControl_TS::var_info;
    if (!attitude_control) {
        AP_BoardConfig::allocation_error("attitude_control");
    }

    AP_Param::load_object_from_eeprom(attitude_control, atc_var_info);
    ...
    weathervane = new AC_WeatherVane(pos_control);
    if (!weathervane) {
...
{    
    ...
    if ((throttle_in <= 0) && !air_mode_active() && (AP_HAL::millis() - last_motors_active_ms >= 400)) {
...
void QuadPlane::get_pilot_desired_lean_angles(float &roll_out_cd, float &pitch_out_cd, float angle_max_cd, float angle_limit_cd) const
{
    if (plane.failsafe.rc_failsafe || plane.failsafe.throttle_counter > 0 ||
        (!option_is_set(QuadPlane::OPTION::REPOSITION_LANDING) && plane.control_mode == &plane.mode_qland)) {
        roll_out_cd = 0;
        pitch_out_cd = 0;
        return;
    }
...
float QuadPlane::get_pilot_input_yaw_rate_cds(void) const
{
    ...
    if (plane.control_mode != &plane.mode_qstabilize &&
        ...
        !(option_is_set(QuadPlane::OPTION::REPOSITION_LANDING) && in_vtol_land_descent())) {
        return 0;
    }
...
void SLT_Transition::update()
{
    ...
    case TRANSITION_AIRSPEED_WAIT: {
        ...
        if (have_airspeed && aspeed > plane.aparm.airspeed_min && !quadplane.assisted_flight &&
            plane.relative_ground_altitude(plane.g.rangefinder_landing) >= quadplane.tran_alt_max) {
            transition_state = TRANSITION_TIMER;
...
void QuadPlane::vtol_position_controller(void)
{
    ...
    const float position2_target_speed = wp_nav->get_default_speed_xy() * 0.01;

    // distance that we switch to QPOS_POSITION2
    const float position2_dist_threshold = sq(position2_target_speed) / (2 * transition_decel);
    ...
    case QPOS_POSITION1: {
        ...
        const float target_speed = position2_target_speed;
    ...
    case QPOS_POSITION2: {
        ...
        if (plane.control_mode == &plane.mode_qrtl || plane.control_mode == &plane.mode_guided || vtol_loiter_auto) {
            int32_t target_altitude_cm = plane.relative_target_altitude_cm();
            Location origin;
            if (ahrs.get_origin(origin))
                target_altitude_cm += plane.home.alt - origin.alt;
            float zero = 0;
...
void QuadPlane::takeoff_controller(void)
{
    ...
    */
    ...
    if (no_navigation) {
        ...
        pos_control->relax_velocity_controller_xy();
        // move pos target to stopping point
        Vector3p stopping_point;
        pos_control->get_stopping_point_xy_cm(stopping_point.xy());
        Location stopping_loc(stopping_point, Location::AltFrame::ABOVE_ORIGIN);
        ...
        pos_control->set_pos_target_xy_cm(poscontrol.target_cm.x, poscontrol.target_cm.y);
    ...
    int32_t next_ground_course_cd = -1;
    AP_Mission::Mission_Command cmd;
    if (plane.control_mode == &plane.mode_auto &&
        plane.mission.get_next_nav_cmd(plane.mission.get_current_nav_index() + 1, cmd) &&
        (cmd.content.location.lat != 0 || cmd.content.location.lng != 0))
        next_ground_course_cd = plane.current_loc.get_bearing_to(cmd.content.location);

    float yaw_align_height = plane.next_WP_loc.alt;
    if (is_positive(attitude_control->get_slew_yaw_max_degs()))
        yaw_align_height -= 180 / attitude_control->get_slew_yaw_max_degs() * wp_nav->get_default_speed_up();
    ...
    if (next_ground_course_cd != -1 && plane.current_loc.alt > yaw_align_height)
        ...
        attitude_control->input_euler_angle_roll_pitch_euler_rate_yaw(plane.nav_roll_cd,
                                                                      plane.nav_pitch_cd,
                                                                      no_navigation ? 0 : get_pilot_input_yaw_rate_cds() + get_weathervane_yaw_rate_cds());
...
bool QuadPlane::land_detector(uint32_t timeout_ms)
{
    ...
    const uint32_t now = AP_HAL::millis();
    float height = plane.relative_ground_altitude(false);
    ...
    */
    if (plane.control_mode == &plane.mode_qacro ||
        ...
        if (rc_fwd_thr_ch == nullptr) {
            ...
            float fwd_thr = 100 - rc_fwd_thr_ch->percent_input();
...
float QuadPlane::get_weathervane_yaw_rate_cds(void)
{
    ...
    */
    ...
    if (weathervane->get_yaw_out(wv_output,
                                     is_zero(get_pilot_input_yaw_rate_cds()) ? 0 : plane.channel_rudder->get_control_in(),
                                     plane.relative_ground_altitude(plane.g.rangefinder_landing),
                                     pos_control->get_roll_cd(),
                                     pos_control->get_pitch_cd(),
                                     is_takeoff,
                                     in_vtol_land_descent())) {
        float yaw_rate_max_cds = command_model_pilot.get_rate() * 100;
        return constrain_float(wv_output * transition->weathervane_gain(), -yaw_rate_max_cds, yaw_rate_max_cds);
    }
...
void QuadPlane::guided_update(void)
{
    if (plane.control_mode == &plane.mode_guided && guided_takeoff && plane.current_loc.alt < plane.next_WP_loc.alt) {
        ...
        vtol_position_controller();
        if (plane.guided_state.vtol_type == VtolType::Land && poscontrol.get_state() >= QPOS_POSITION2)
            verify_vtol_land();
    }
}
...
bool QuadPlane::guided_mode_enabled(void)
{
    ...
    if (plane.control_mode == &plane.mode_guided) {
        if (plane.guided_state.vtol_type == VtolType::FixedWing)
            return false;
        if (plane.guided_state.vtol_type == VtolType::MultiCopter || plane.guided_state.vtol_type == VtolType::Land)
            return true;
    }
...
bool QuadPlane::do_user_takeoff(float takeoff_altitude)
{
    ...
    if (!hal.util->get_soft_armed() && !AP::arming().arm(AP_Arming::Method::MAVLINK)) {
...
{
    return in_vtol_land_sequence() && poscontrol.get_state() == QPOS_LAND_FINAL;
}
...
bool QuadPlane::in_vtol_land_sequence(void) const
{
    return (plane.control_mode == &plane.mode_qland ||
            plane.control_mode == &plane.mode_qrtl ||
           (plane.control_mode == &plane.mode_guided && plane.auto_state.vtol_loiter && plane.guided_state.vtol_type == VtolType::Land) ||
...
void QuadPlane::check_alt_fence(void)
{
    ...
    if (fence_alt <= 0 || plane.control_mode == &plane.mode_manual) {
        return;
    }
...
{
    if (land_max_alt_m > 0)
        return plane.home.alt + land_max_alt_m * 100;
    else
        return plane.get_RTL_altitude_cm();
}
...
{
    float angle_limit_cd = quadplane.aparm.angle_max;
    ...
    if (quadplane.back_trans_pitch_limit_ms > 0) {
        ...
        if (last_fw_mode_ms == 0 || dt > limit_time_ms) {
            ...
            angle_limit_cd = linear_interpolate(0, quadplane.aparm.angle_max, dt, 0, limit_time_ms);
    }
...
float SLT_Transition::weathervane_gain()
{
    int tran_time = quadplane.back_trans_pitch_limit_ms;
    if (tran_time > 0)
        return linear_interpolate(0.0f, 1.0f, AP_HAL::millis() - last_fw_mode_ms, tran_time / 2, tran_time);
    return 1.0f;
}
