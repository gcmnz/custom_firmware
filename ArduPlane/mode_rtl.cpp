...
bool ModeRTL::switch_QRTL(bool check_loiter_target)
{
   ...
   if (plane.quadplane.rtl_mode == QuadPlane::RTL_MODE::QRTL_ALWAYS) {
    ...
    if (plane.height_above_target() > 10.0f)
        return false;
    ...
    if ( (check_loiter_target && plane.nav_controller->reached_loiter_target()) ||
        ...
        int32_t landable_alt_cm = plane.quadplane.get_landable_alt_cm();
        if (plane.next_WP_loc.alt > landable_alt_cm) {
            // RTL destination reached, descend to landable altitude
            plane.next_WP_loc.alt = landable_alt_cm;
            if (plane.height_above_target() > 10.0f)
                return false;
        }
        /*
          ...
          RTL_RADIUS
         */
        plane.rally.set_override_loc(plane.next_WP_loc);
        bool had_target_override = plane.rtl.target_override;
        plane.set_mode(plane.mode_qrtl, ModeReason::RTL_COMPLETE_SWITCHING_TO_VTOL_LAND_RTL);
        plane.rtl.target_override = had_target_override;
        plane.rally.clear_override_loc();
        return true;
    }
