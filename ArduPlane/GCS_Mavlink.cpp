...
MAV_RESULT GCS_MAVLINK_Plane::handle_command_int_do_reposition(const mavlink_command_int_t &packet)
{
    ...
    if (((int32_t)packet.param2 & MAV_DO_REPOSITION_FLAGS_CHANGE_MODE) ||
        ...
        plane.set_guided_WP(requested_position, vtol_type);
...
MAV_RESULT GCS_MAVLINK_Plane::handle_command_int_nav_vtol_land(const mavlink_command_int_t &packet)
{
    ...
    requested_position.alt = plane.quadplane.get_landable_alt_cm();
    ...
    if (hal.util->get_soft_armed() && plane.set_mode(plane.mode_guided, ModeReason::GCS_COMMAND)) {        
        plane.setup_terrain_target_alt(requested_position);
        plane.set_guided_WP(requested_position, VtolType::Land);
        return MAV_RESULT_ACCEPTED;
    }
...
MAV_RESULT GCS_MAVLINK_Plane::handle_command_int_packet(const mavlink_command_int_t &packet)
{
    ...
    case MAV_CMD_NAV_VTOL_LAND:
        return handle_command_int_nav_vtol_land(packet);
