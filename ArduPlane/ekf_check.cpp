...
void Plane::ekf_check()
{
    ...
    if (over_threshold || !has_position) {
    ...
    if (ekf_check_state.failsafe_on) {
        ...
        if (control_mode == &mode_qland)
            quadplane.qland_no_gps = true;
        else if ((control_mode == &mode_auto || control_mode == &mode_guided) && !quadplane.in_vtol_land_descent()) {
            // try to fly home
            set_mode(mode_rtl, ModeReason::EKF_FAILSAFE);
        }
        else if (control_mode == &mode_qloiter || control_mode == &mode_qautotune) {
            // the pilot is controlling via sticks so fallback to QHOVER
            set_mode(mode_qhover, ModeReason::EKF_FAILSAFE);
        }
        else {
            // the pilot is not controlling via sticks so switch to QLAND
            set_mode(mode_qland, ModeReason::EKF_FAILSAFE);
        }
    }
}
