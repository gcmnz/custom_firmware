...
const AP_Param::GroupInfo AP_TECS::var_info[] = {
    ...
    // @Increment: 0.1
    // @User: Advanced
    AP_GROUPINFO("PTCH_DAMP", 10, AP_TECS, _ptchDamp, 0.1f),
    ...
    AP_GROUPINFO("HGT_P", 41, AP_TECS, _hgt_p_gain, 0.2f),
    AP_GROUPINFO("HGT_FF", 42, AP_TECS, _hgt_ff_gain, 0.3f),

    AP_GROUPINFO("TAS_P", 43, AP_TECS, _TAS_p_gain, 0.2f),
    AP_GROUPINFO("THR_I", 44, AP_TECS, _thr_i_gain, 0.1f),
...
void AP_TECS::update_50hz(void)
{
    ...
    float temp = rotMat.c.x * GRAVITY_MSS + (_ahrs.get_rotation_autopilot_body_to_vehicle_body() * (_ahrs.get_accel() - _ahrs.get_accel_bias())).x;
...
void AP_TECS::_update_speed_demand(void)
{
    ...
    _TAS_rate_dem = constrain_float((_TAS_dem_adj - _TAS_state) * _TAS_p_gain, velRateMin, velRateMax);
}
...
void AP_TECS::_update_height_demand(void)
{
    ...
    if (_landing.is_flaring()) {
        ...
        _hgt_rate_dem = _hgt_p_gain * (_hgt_dem_adj - _height) + _hgt_ff_gain * (_hgt_dem_adj - _hgt_dem_adj_last) / 0.1f;
        _hgt_rate_dem = constrain_float(_hgt_rate_dem, -max_sink_rate, _maxClimbRate);
        _flare_counter = 0;
    }
...
void AP_TECS::_update_throttle_with_airspeed(void)
{
    ...
    if (_flight_stage == AP_Vehicle::FixedWing::FLIGHT_VTOL || is_zero(_spdWeight)) {
        /*
    ...
    if (_flags.underspeed) {
        ...
        _integTHR_state = _integTHR_state + (_STE_error * _thr_i_gain) * _DT * K_STE2Thr;
...
void AP_TECS::_update_pitch(void)
{
    ...
    SPE_weighting = constrain_float(SPE_weighting, 0.0f, 1.0f);
    SKE_weighting = constrain_float(SKE_weighting, 0.0f, 1.0f);
