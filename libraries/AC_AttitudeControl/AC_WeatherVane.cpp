...
// Constructor
AC_WeatherVane::AC_WeatherVane(AC_PosControl *pos_control) :
...
bool AC_WeatherVane::get_yaw_out(float &yaw_output, const int16_t pilot_yaw, const float hgt, const float roll_cdeg, const float pitch_cdeg, const bool is_takeoff, const bool is_landing)
{
    ...
    */
    ...
    switch (dir) {
        ...
        case Direction::NOSE_IN_PSC: {
            Vector2f psc_accel_xy = _pos_control->get_accel_target_cmss().xy();
            float min_accel = GRAVITY_MSS * 100.0f * tanf(radians(_min_dz_ang_deg));
            float accel_length = psc_accel_xy.length();
            if (accel_length > min_accel) {
                float max_accel = GRAVITY_MSS * 100.0f * tanf(radians(_pos_control->get_lean_angle_max_cd()*0.01f));
                float thrust_bearing_cds = RadiansToCentiDegrees(wrap_2PI(atan2f(psc_accel_xy.y, psc_accel_xy.x)));
                float yaw_error = wrap_180_cd(thrust_bearing_cds - AP::ahrs().yaw_sensor);
                float accel_gain = linear_interpolate(0.0f, 1.0f, accel_length, min_accel, max_accel);
                output = yaw_error * accel_gain;
            }
            dir_string = "nose in psc";
            break;
        }
    }
