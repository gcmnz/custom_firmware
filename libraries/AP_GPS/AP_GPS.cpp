...
const AP_Param::GroupInfo AP_GPS::var_info[] = {
    ...
    AP_GROUPINFO("_VALID_VACC", 55, AP_GPS, _validate_max_vacc, 0),
    AP_GROUPINFO("_VALID_SPEED", 56, AP_GPS, _validate_max_speed_ms, 0),
    AP_GROUPINFO("_VALID_DTIME", 57, AP_GPS, _validate_max_dt_sec, 0),
...
void AP_GPS::update_instance(uint8_t instance)
{
    ...
    if (!result) {
        ...
        if (state[instance].status > NO_FIX && !is_fix_valid(instance))
            state[instance].status = NO_FIX;
        ...
        if (state[instance].status >= GPS_OK_FIX_2D && !_force_disable_gps) {
            ...
            timing[instance].last_valid_location = state[instance].location;
        }
...
bool AP_GPS::is_fix_valid(uint8_t instance) const
{
    // if disarmed, accept whatever
    ...
    if (is_positive(_validate_max_vacc) && istate.vertical_accuracy > _validate_max_vacc)
        return false;

    if (is_positive(_validate_max_speed_ms)) {
        Vector3f diff = istate.location.get_distance_NED(timing[instance].last_valid_location);
        float dt = MAX((AP_HAL::millis() - timing[instance].last_fix_time_ms) * 0.001f, 1.0f);
        if (diff.length() > dt * _validate_max_speed_ms)
            return false;
    }

    uint64_t rtc_clock_us;
    if (is_positive(_validate_max_dt_sec) && AP::rtc().get_utc_usec(rtc_clock_us)) {
        uint64_t fix_time_us = istate_time_to_epoch_ms(istate.time_week, istate.time_week_ms) * 1000ULL;
        const uint64_t max_dt_us = _validate_max_dt_sec * 1000000ULL;
        if (fix_time_us < rtc_clock_us - max_dt_us || fix_time_us > rtc_clock_us + max_dt_us)
            return false;
    }
