...
float AP_MotorsMulticopter::get_current_limit_max_throttle()
{
    ...
    // limit max throttle
    return get_throttle_hover() + ((_thr_max - get_throttle_hover()) * _throttle_limit);
}
...
void AP_MotorsMulticopter::update_lift_max_from_batt_voltage()
{
    ...
    // filter at 0.5 Hz
    _batt_voltage_filt.apply(_batt_voltage_resting_estimate / _batt_voltage_max, _dt);

    // assume spin_max increase will compensate for lift
    _lift_max = 1.0f;
}
...
float AP_MotorsMulticopter::thrust_to_actuator(float thrust_in) const
{
    ...
    float thrust_out = _spin_min + (_spin_max - _spin_min) * apply_thrust_curve_and_volt_scaling(thrust_in);
    float battery_scale = 1.0f / MAX(_batt_voltage_filt.get(), MAX(_spin_max, 0.6f));
    return constrain_float(thrust_out * battery_scale, 0.0f, 1.0f);
}
